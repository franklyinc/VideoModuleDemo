...
// //       FranklyVideoPlayerList[0].playerInfo.loadInitialVideo(true);
  ...
  render(){
    ...
    var vidList = this.props.streamUrls.map(function(streamUrl, index){
      ...
      return (
        <div style={{gridArea: 'vid' + index}}>
          <div style={vidThumbStyles} className="vidThumb" onClick={() => this.updateVideo(index)} >
    ...
    for (var i = 1; i <= 3; i++) { // add empty cells if less than 3 streams
      if (this.props.streamUrls.length < i) {
